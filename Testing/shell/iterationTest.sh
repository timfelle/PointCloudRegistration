...
Prepare()
{
	...
	ROTATION=1,0,0 \
	./GenerateData seal_left.ply seal_left.ply
	TRANSLATION=0,-600,-200 \
	./GenerateData seal_right.ply seal_right.ply
}
...
{
	SETS="'seal'"
	I="1 5 10 20 50 70 100 150 300 1000"
	for i in $I ; do
		SETS+=",'res$i-'"
		FGR_MAXITER=$i \
		OUTPUT_NAME="res$i-" \
		FGR_VERSION=open3d \
		./Registration.exe seal
	done
}
...
Visualize()
{
	...
	matlab $MATOPT \
	-r "addpath('$MAT');displayRegistration({$SETS},'dat','fig');exit;"
	matlab $MATOPT \
	-r "addpath('$MAT');animateRegProcess({$SETS},'dat','fig');exit;"
}
