...
Prepare()
{
	...
	mkdir -p dat
	cp -ft dat/ $DAT/bunnyPartial*

	TRANSLATION=0.15,0,0.15 \
	./GenerateData.exe bunnyPartial2.ply bunnyPartial2.ply
}
...
Visualize()
{
	...
	matlab $MATOPT \
	-r "addpath('$MAT');displayRegistration({'bunny','result'},'dat','fig');
		displayCorrespondences({'corr'},'dat','fig');
		displayCorrespondences({'corr'},'dat','fig',true);exit;"
}
